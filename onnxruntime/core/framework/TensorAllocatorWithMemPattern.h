...
class TensorAllocatorWithMemPattern : public ITensorAllocator {
 private:
  ...
  common::Status AllocatePlannedBuffers() {
    ...
    for (size_t i = 0; i < location_len; ++i) {
      ...
      auto alloc = utils::GetAllocator(exec_providers_, location);
      if (!alloc)
 ...
 public:
  TensorAllocatorWithMemPattern(const SequentialExecutionPlan& execution_plan, const ExecutionProviders& exec_providers,
